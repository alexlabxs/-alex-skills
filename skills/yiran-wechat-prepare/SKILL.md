...
- `source/story.md` 已存在
- 生图参考文件已安装：
  - 非童心系列：`~/.claude/plugins/marketplaces/ian-xiaohei/skills/ian-xiaohei-illustrations/references/`
  - 童心系列：`~/.claude/skills/tongxin-xiaodian-illustrations/references/`（由 alex-skills 安装）
...
读取 `source/story.md` 的 `series:` 字段，决定用哪个 IP：

| 系列 | IP | 参考文件来源 |
|------|----|------------|
| 🧸 童心 | **小点**（白色圆点+淡彩光晕） | `tongxin-xiaodian-illustrations/references/` |
| 其他（深度/通识/成长/社会） | **小黑**（黑色实心小怪物） | `ian-xiaohei-illustrations/references/` |

### 第二步：加载参考文件

**非童心系列** → 加载 `ian-xiaohei-illustrations` 的 4 个参考文件：
- `references/style-dna.md` — 颜色规则、留白比例、禁忌
- `references/xiaohei-ip.md` — 小黑动作库
- `references/prompt-template.md` — prompt 结构
- `references/qa-checklist.md` — 生成后验证

**童心系列** → 加载 `tongxin-xiaodian-illustrations` 的 2 个参考文件：
- `references/xiao-dian-ip.md` — 小点 IP 定义、光晕颜色规则、动作库
- `references/prompt-template.md` — 生图 prompt 结构

### 第三步：逐张生成

使用 `baoyu-image-gen` + DashScope：

- 内文配图：`--ar 16:9 --quality 2k`
- 封面图：`--size "1920*817"`
- prompt 按对应该 IP 的 `prompt-template.md` 写
- 每张图只讲一个核心结构/瞬间

### 第四步：QA 验证

**非童心系列** → 用 ian-xiaohei 的 QA checklist：
- 小黑是否核心动作？
- 画面 ≤ 60%？
- 无左上角标题？
- 无纸纹/渐变/阴影？
...
| ❌ 配图用错 IP | Step 3 先读 series 字段，童心→小点，其他→小黑 |
| ❌ 童心配图不可爱 | 小点的淡彩光晕必须手绘感，不要发光特效 |
| ❌ 配图风格走样 | Step 3 加载对应该系列的 IP 参考文件后生图 |
