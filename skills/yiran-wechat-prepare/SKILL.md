...
- 本地环境：bun 已安装
- `source/story.md` 已存在
- `yiran-illustrate` skill 已安装（位于 `~/.claude/skills/yiran-illustrate/`）
...
调用 **`yiran-illustrate`** 技能来完成配图。
不要在这里内联写分支逻辑——所有系列的配图规则在 `yiran-illustrate/series/{name}/` 下。

具体做法：

1. 加载 `yiran-illustrate` 的 `config/series-map.md` 确定 series → 目录映射
2. 加载对应系列目录的全部参考文件（ip.md, style-dna.md, prompt-template.md, qa-checklist.md）
3. 按 `yiran-illustrate` SKILL.md 的 Step 3-5 执行：出 shot list → 逐张生成 → QA 验证
4. 保存到 `01-wechat/assets/`

> **为什么这样设计**：yiran-illustrate 统一管理 5 个系列的 IP 和视觉风格。
> 加新系列只需在它的 `series/` 下加目录，不用改此技能。
...
| ❌ 配图用错 IP | Step 3 通过 yiran-illustrate/series-map 自动匹配，无需手动分支 |
| ❌ 配图风格走样 | Step 3 加载 yiran-illustrate/series/{name}/ 下的参考文件后生图 |
